...
files=ftp.nlst()
ftp.cwd(os.path.join(ftp.pwd(),files[-8]))
file_list=ftp.nlst() 
print(file_list)
#%%
...
def download_ecmwf(path,filepatern):
    """
    ...
    ftp.login("wmo","essential")
    files=ftp.nlst()
    ftp.cwd(os.path.join(ftp.pwd(),files[-7]))
    file_list=ftp.nlst()    
    for files in file_list:
        if filepatern in files:
...
#%%
def pre_process_ecmwf(path):
    """
    preprocess ecmwf forecast data downloaded above
    """
    path_ecmwf=os.path.join(path,'forecast/ecmwf/')
    path_input=os.path.join(path,'forecast/Input/')
    ...
    for ecmwf_file in ecmwf_files:
        ...
        try:
            ...
            df_forecast.to_csv(os.path.join(path_input,f_name+'_%s.csv'%STORMNAME),index=True)
        except:
            pass
    
#%%
path='C:/Users/ATeklesadik/OneDrive - Rode Kruis/Documents/documents/Typhoon-Impact-based-forecasting-model/'
filepatern='_track_NURI_'
download_ecmwf(path,filepatern)
pre_process_ecmwf(path)
