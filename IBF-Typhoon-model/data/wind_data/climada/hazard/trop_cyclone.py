...
class TropCyclone(Hazard):
    ...
    def set_from_tracks(
        ...
        LOGGER.info(
            "Mapping %s tracks to %s coastal centroids.",
            str(tracks.size),
    ...
    def set_climate_scenario_knu(self, ref_year=2050, rcp_scenario=45):
        ...
        haz_cc.tag.description = (
            "climate change scenario for year %s and RCP %s "
    ...
    def _tc_from_track(
        ...
        windfields, reachable_centr_idx = compute_windfields(
            track, coastal_centr, mod_id, metric=metric
        )
        ...
        if store_windfields:
            ...
            indices = np.zeros(
                (npositions, n_reachable_coastal_centr, 2), dtype=np.int64
            )
            ...
            windfields_sparse = sparse.csr_matrix(
                (windfields.ravel(), indices, indptr),
        ...
        new_haz.basin = [
            track.basin if isinstance(track.basin, str) else str(track.basin.values[0])
        ]
        return new_haz
...
def compute_windfields(track, centroids, model, metric="equirect"):
    ...
    v_centr_normed[close_centr_msk] = (
        v_centr[close_centr_msk] / d_centr[close_centr_msk, None]
    )
    ...
    if model == 0:
        hol_b = _bs_hol08(
            v_trans_norm[1:], t_env[1:], t_cen[1:], prev_pres, t_lat[1:], t_tstep[1:]
        )
    else:
        raise NotImplementedError

    # derive angular velocity
    v_ang_norm = _stat_holland(
        d_centr[1:],
        t_rad[1:],
        hol_b,
        t_env[1:],
    ...
    v_ang[close_centr_msk[1:]] = (
        v_ang_norm[close_centr_msk[1:], None] * v_ang_dir[close_centr_msk[1:]]
    )
...
def _stat_holland(d_centr, r_max, hol_b, penv, pcen, lat, close_centr):
    ...
    sqrt_term = (
        100 * hol_b / rho * r_max_norm * (penv - pcen) * np.exp(-r_max_norm)
        + r_coriolis ** 2
    )
