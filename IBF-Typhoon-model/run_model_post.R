...
typhoon_hazard <- wind_grid %>%
  ...
  dplyr::mutate(
    ...
    gust_dur = 0,
    sust_dur = 0,
    vmax_gust = v_max * SUS_TO_GUST*1.21,
    vmax_gust_mph = v_max * SUS_TO_GUST * MS_TO_MPH*1.21,
    vmax_sust_mph = v_max * MS_TO_MPH*1.21,
    vmax_sust = v_max
  ) %>%
  dplyr::select(
    Mun_Code, vmax_gust,
...
df_impact_forecast%>%dplyr::select(Damaged_houses) %>%
...
df_impact_dist50 <- aggregate(
  ...
  dplyr::mutate(probability_dist50 = 100 * x / 1) %>%
  ...
  left_join(
    aggregate(
      ...
      dplyr::mutate(WEA_dist_track = x / 1) %>%
...
track <- track_interpolation(TRACK_DATA) %>%
  dplyr::mutate(Data_Provider = "IBTracks")


####################################################################################################
# ------------------------ Make and save distance and impact maps

impact_map <- Make_maps_post(php_admin1,
                      event_impact,
                      track,
                      TYF = "ECMWF",
                      Typhoon_stormname,
...
tmap_save(impact_map,
          filename = paste0(Output_folder, "Post_event_Average_Impact", "_", forecast_time, "_", Typhoon_stormname, ".png"),
...
write.csv(data.frame(event_impact) %>% dplyr::select(-c(geometry)),
  file = paste0(Output_folder, "Post_Average_Impact", "_", forecast_time, "_", Typhoon_stormname, ".csv")
)
