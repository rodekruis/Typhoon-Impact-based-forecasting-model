...
from util.files_handler import get_file_names
from util.wind_utils import *
...
def fetch_ecmwf(path=None, files=None,target_dir=None):
    """
    ...
    if path is None and files is None:
        files = fetch_bufr_ftp(target_dir=target_dir)
    elif files is None:
...
def read_one_bufr_tc(file, id_no=None, fcast_rep=None):
    ...
    if fcast_rep is None:
        fcast_rep = _find_delayed_replicator(
