...
#setwd('/home/fbf')
# ------------------------ calculate  variables  -----------------------------------
typhoon_events <- read.csv('/home/fbf/forecast/typhoon_info_for_model.csv')
grid_points_adm3<-read.csv("/home/fbf/data-raw/grid_points_admin3.csv", sep=",")
# ------------------------ import boundary layer   -----------------------------------
php_admin3 <- st_read(dsn='/home/fbf/data-raw',layer='phl_admin3_simpl2')
php_admin1 <- st_read(dsn='/home/fbf/data-raw',layer='phl_admin1_gadm_pcode')
...
if (dim(typhoon_events)[1]>0) {
  for(i in 1:nrow(typhoon_events)){
    ...
    if (nrow(Landfall_check) > 0){
      ...
      if(rainfall_error=='True'){
        
        print('--------------NOTE: Using Rainfall Backup')
      ...
      mode_classification <- readRDS("/home/fbf/models/final_model.rds")
      ...
      mode_continious <- readRDS("/home/fbf/models/final_model_regression.rds")
  ...
  Prediction is about completely damaged houses only\n",'Estimated time to Landfall: ',dt,' PST (',time_for_landfall,' hrs)')
      ...
      tmap_save(map1, filename = paste0('/home/fbf/forecast/Impact_',as.vector(TRACK_DATA[1,]$YYYYMMDDHH),'_',as.vector(TRACK_DATA[1,]$STORMNAME),'.png'))
      write.csv(Impact, file = paste0('/home/fbf/forecast/Impact_',as.vector(TRACK_DATA[1,]$YYYYMMDDHH),'_',as.vector(TRACK_DATA[1,]$STORMNAME),'.csv'))
      
      file_names<- c(paste0('/home/fbf/forecast/Impact_',as.vector(TRACK_DATA[1,]$YYYYMMDDHH),'_',as.vector(TRACK_DATA[1,]$STORMNAME),'.png'),
      ...
      write.table(file_names, file = '/home/fbf/forecast/file_names.csv',append=TRUE, col.names = FALSE)
