...
df_imact_forecast_85 <- df_imact_forecast_all%>%filter(region%in%c('PH05','PH08'))
...
typhoonname='bopha' #df_imact_forecast_all%>% filter(GEN_typhoon_name==typhoonname)
...
df_imact_forecast_85<-df_imact_forecast_85%>%dplyr::mutate(dist50=ifelse(WEA_dist_track >= 50,0,1),
                                                         impact1=ifelse(impact > 100,100,impact),
                                                         Damaged_houses=as.integer(GEO_n_households*impact1*0.01),
)%>%
  filter(WEA_dist_track<100)%>%dplyr::select(index,
...
df_imact_forecast_85%>%group_by(GEN_typhoon_name,GEN_typhoon_id)%>%
