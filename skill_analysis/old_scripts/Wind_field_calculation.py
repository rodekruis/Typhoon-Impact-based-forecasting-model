...
for tr in fcast.data:
 ...
 #%%     
df_=df['centroid_id'].values
df_=df_.tolist()*53
df_track=pd.DataFrame({
    'centroid_id': df_,
    'dis_track': distan_track,
    })
df_track=df_track[df_track['dis_track']<300]

df_track=df_track.join(df.set_index('centroid_id'), on='centroid_id')

  #%%  
df_intensity = pd.concat(list_intensity)
df_intensity=df_intensity.set_index('centroid_id')

df2=df_intensity.join(df_track.set_index('centroid_id'), on='centroid_id')
...
admin=gpd.read_file("C:/data/data-raw/phl_admin3_simpl2.shp")
