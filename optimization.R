...
for (iter in 1:10000){
  param <- list(booster =sample(c("gbtree","gbdart"),1),
...
#create tasks
traintask <- makeClassifTask (data = df_train, target = "more_than_10_perc_damage_")
testtask <- makeClassifTask (data = df_test, target = "more_than_10_perc_damage_")
...
eval_function_XGBoost_Linear <- function(x, data,train_settings){
  ...
  suppressWarnings(#Create dataframe with proportion of each solid component
    XGBoost_Linear_model <- caret::train(more_than_10_perc_damage_ ~.,
                                       data = df_train,
                                       method = "xgbDART",
